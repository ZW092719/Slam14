...
**从相机坐标系向世界坐标系的变换，也就说将世界坐标系下的点转换到相机坐标系下的点的变换矩阵**
...
其中第一个参数是生成的源代码文件名称，那么第二个参数可以加什么？
...
有了这个之后不需要再写include_directories

那么这个名称如何找？同样还是使用locate，之后找到sophusTargets这种targets文件，进去之后找到具体代码Sophus :: Sophua
...
3.target_link_libraries(执行文件名, 库：：库)

这个主要是找target文件



add_subdirectory(一个子文件夹)

https://blog.csdn.net/sinat_31608641/article/details/122660652

这个主要写在主文件夹子下的cmakelists，加上这个就会编译自文件夹的程序

#### 评估轨迹的误差

实际工程经常需要评估**估计轨迹**与**真实轨迹**的差异来评估算法精确度。

真实轨迹往往通过更高精度的系统获得，估计轨迹由算法得到。
...
$$
gt的变换矩阵与估计位姿的变换矩阵相乘之后再通过sophus中的.log()转换为李代数，再计算其范数
...
这实际上是每个位姿李代数的均方根误差（RMSE），可以刻画两条轨迹的旋转和平移误差。

仅考虑平移误差有绝对平移误差（ATE）：
$$
\mathrm{ATE}_{\mathrm{trans}}=\sqrt{\frac{1}{N}\sum_{i=1}^N\|\mathrm{trans}(T_{\mathrm{gt},i}^{-1}T_{\mathrm{esti},i})\|_2^2},
$$
trans表示取内部变量的平移部分

此外还有相对位姿误差（RPE），为ℹ时刻到i+deltaℹ时刻：
$$
\mathrm{RPE}_{\mathrm{all}}=\sqrt{\frac1{N-\Delta t}\sum_{i=1}^{N-\Delta t}\|\log\left(\left(T_{\mathrm{gt},i}^{-1}T_{\mathrm{gt},i+\Delta t})\right)^{-1}\left(T_{\mathrm{est},i}^{-1}T_{\mathrm{est},i+\Delta t}\right)\right)^{\vee}\|_{2}^{2},}
$$
利用sophus实现这部分计算：

关于代码的解析可以看：

https://blog.csdn.net/joun772/article/details/109285750
...
本讲讨论机器人如何观测外部世界，也就是**观测方程**部分。在视觉slam中也就是**相机成像**过程

现实世界中的物体反射或发出的光线穿过相机光心后投影在相机的成像平面。相机感光器接收到光线后，产生测量值，得到了像素最终形成照片
...
**这个过程是我们把成像平面等价的放到了相机前方，和三维空间点放在相机的一侧**
$$
\frac Zf=\frac X{X^{\prime}}=\frac Y{Y^{\prime}}.
$$
...
$$
其中*Z为物体到相机的垂直距离，也就是相机坐标系下物体的Z方向的坐标（深度信息），f*为相机焦距

**像素坐标系的定义：**

原点o‘通常定义在图像的左上角，u轴向右，v轴向下，像素坐标系与成像平面之间相差了一个缩放和原点的平移
...
有的相机厂商会告诉你相机内参而有的则不会需要自己确定，也就是所谓的**标定。**标定算法比如：张正友标定法。
...
这个位姿也就是slam要带估计的目标代表着机器人的轨迹，

> 这个姿态就是相机坐标系到世界坐标系的变换描述，另一个含义就算世界坐标系下的点到相机坐标系下的点需要进行转换的变换矩阵
...
即世界坐标中的一个物体Xw，Yw，Zw，通过变换矩阵先变换到相机坐标系下（注意由于变换矩阵是4x4的因此这其中涉及到一次齐次坐标变换），得到Xc，Yc，Zc；之后把他除以Zc深度信息，得到在相机归一化平面上的坐标；之后再左乘相机的内参矩阵便得到像素坐标系下的坐标！很简单
...
畸变发生在归一化平面上的坐标转为像素平面的过程中，**下面公式中等号右侧的x，y是在相机坐标系下归一化后的坐标，因此是在此时进行的修正**
...
$$
**d为视差**描述同一个点在左右目上成像的距离，d最小为1个像素，因此数目**能测量的深度Z有最大值：fb**

视差与深度成反比

虽然距离公式简单，但d不容易计算，难在视差的计算
...
需要确定相机的内参因而需要进行标定，这样才可以更好的估计相机的外参

Opencv，matlab，Kalibr
...
**图像的高度对应的是数组的行数，图像的宽度对应的是数组的列数**

访问某一个位置的像素就是访问数组对应的下标，但是需要！：

> 对于一张图像原点在左上角，右侧为x轴，向下为y轴，**因此访问图像上的像素（x，y），就是访问数组（y，x）**
>
> 一定要注意这个顺序，否则极容易产生越界
...
>
> **灰度图对应着0～255的值共256个数，因此使用8位bit来记录，也就是一个字节**

深度图：

> 深度图是一种特殊的图像，表示场景中每个像素到摄像机的距离。深度信息通常以灰度或伪彩色编码的形式显示。
>
> 深度图像中的每个像素值表示该像素与摄像机之间的距离。这个距离通常以毫米为单位因而超过了255。这时则采用**16bit来记录，（C++中的unsigned short）来记录**，也就是位于0～65535的值，换算成米为65米

彩色图：

> 彩色图是包含颜色信息的图像，通常由三个**通道**组成，分别代表红色（R）、绿色（G）和蓝色（B）。通过不同强度的红、绿、蓝组合可以表示各种颜色。
>
> 每个通道都有8bit组成，这样情况下一个像素占据24位空间
>
> 在OPenCV中通道默认顺序是**B、G、R**，也就是得到一个24位的像素时，前8位为蓝，中间8位为绿，最后8位为红
...
（3）dpkg -i xxx.deb  适用于安装应用

（4）source源代码安装

**2.一些基础操作**

**（1）关于main(int argc,char** argv)**

这个定义main的方式主要用于在命令行运行给main函数传参

argc：表示的是命令行中具有几个字符串，不在程序文件./执行文件后加别的东西的话就为1，即该程序本身自己的路径（如果添加则变成相应的数量）

argv：是存储字符串的容器，argv[0]存储的是程序本身自己的路径
...
.rows：图像的高
 ...
 图片变量的名称= cv::imread(图像路径) ：图片读入

cv::imshow("窗口名称",图片变量)

**（3）图像每个像素的遍历操作**

对于彩色或灰度图均使用的是uchar类型来存储

使用opencv的特定.ptr指针访问速度最快

获取某一行的指针：

注意获取到的指针是指向第某行，第一个像素，指向0通道的指针

> 因为在c++的opencv中无法进行类似于数组的[] []的访问操作，
>
> **他的cv::Mat默认是二维数组的，相当于一个RGB像素占了三列**

```
for (int y = 0; y < image.rows; i++) {
    uchar* row_ptr = image.ptr<uchar>(y) // 获取第y行的头指针
    for (int x = 0; x < image.cols; x++) {
        uchar* data_ptr = &row_ptr[image.channel() * x] // 获取具体的（x，y）像素坐标对应的数组坐标（y，x）
        // 对应的不是RGB顺序而是BGR
        uchar blue = data_ptr[0];
        uchar green = data_ptr[1];
        uchar red = data_ptr[2];
    }
    // 或者再加一个循环来获取具体通道
...
```
cv::Rect(x,y,w,h)
四个参数，前两个为图像的左上角的坐标，(以左上角为原点建立的坐标系)
后两个是图像的宽与高
他将返回这一个图片的这部分区域
其还具有许多的函数，可结合setTo使用
```

**（5）setTo对像素的三个通道的值进行操作**

setTo直接对三个通道操作

**（6）image.clone()**

真正的克隆拷贝一份数据，在新数据的操作不会影响到原图像

**（7）去畸变操作**

首先拿到的是一张具有畸变的图像，但是我们去畸变操作是在相机坐标系下的归一化平面进行的，因此首先要乘以K逆转换到归一化平面
...
// 计算去畸变后图像的内容
  for (int v = 0; v < rows; v++) {
    for (int u = 0; u < cols; u++) {
      ...
      double x = (u - cx) / fx, y = (v - cy) / fy; // u = fx * x / z + cx 反推回去
      double r = sqrt(x * x + y * y);
      double x_distorted = x * (1 + k1 * r * r + k2 * r * r * r * r) + 2 * p1 * x * y + p2 * (r * r + 2 * x * x);
      double y_distorted = y * (1 + k1 * r * r + k2 * r * r * r * r) + p1 * (r * r + 2 * y * y) + 2 * p2 * x * y;
      double u_distorted = fx * x_distorted + cx;
      double v_distorted = fy * y_distorted + cy;
      image_undistort2.at<uchar>(v, u) = image.at<uchar>((int) v_distorted, (int) u_distorted); // 没有筛选的赋值
      // 赋值 (最近邻插值)
      if (u_distorted >= 0 && v_distorted >= 0 && u_distorted < cols && v_distorted < rows) {
        image_undistort1.at<uchar>(v, u) = image.at<uchar>((int) v_distorted, (int) u_distorted);
      } else {
        image_undistort1.at<uchar>(v, u) = 0;
      }
    }
  }
```

去畸变就可以理解为对于畸变图像的像素点经过公式的映射得到了一个新的像素位置，并将这个新的像素位置上的数据（这个数据还是畸变图像上的，只不过位置变了）赋值给新创建的图像
